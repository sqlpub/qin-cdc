...
func (m *MetaPlugin) Configure(conf map[string]interface{}) error {
	m.StarrocksConfig = &config.StarrocksConfig{}
	var target = conf["target"]
	if err := mapstructure.Decode(target, m.StarrocksConfig); err != nil {
		return err
	}
	return nil
}
...
func (m *MetaPlugin) LoadMeta(routers []*metas.Router) (err error) {
	...
	dsn := fmt.Sprintf(
		...
		m.StarrocksConfig.UserName, m.StarrocksConfig.Password,
		m.StarrocksConfig.Host, m.StarrocksConfig.Port)
	...
	for _, router := range routers {
		...
		m.tables[metas.GenerateMapRouterKey(router.TargetSchema, router.TargetTable)] = table
	}
	return nil
}
